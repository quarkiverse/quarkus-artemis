...
public abstract class BaseArtemisProducerXATest extends BaseArtemisProducerTest {
    ...
    private void testXA(JMSContext context, String address, String queueName) throws JMSException {
        String body = createBody();
        Response response = RestAssured.with().body(body).post(address);
    ...
    private void testRollback(JMSContext context, String address, String queueName) {
        Response response = RestAssured.with().body("fail").post(address);
        Assertions.assertEquals(javax.ws.rs.core.Response.Status.NO_CONTENT.getStatusCode(), response.statusCode());
